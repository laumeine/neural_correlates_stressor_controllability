---
title: "Analyses for 'Don't Stress, It's Under Control: Neural Correlates of Stressor Controllability in Humans'"
...
stress <- stress %>%
  mutate(Stress_duration = case_when(condition == "bas" ~ 0, condition != "bas" ~ Stress_duration)) # set stress durations to zero for baseline condition
...
stressDur <- stress %>%
  ...
  summarise(total_con_stress_dur = mean(total_con_stress_dur),
            ...
            stress_dur_diff = total_con_stress_dur - total_noc_stress_dur)
...
sd(stressDur$stress_dur_diff) # get standard deviation
...
# add stress duration difference score to ratings
ratings <- list(ratings, stressDur[, c(1,4)]) %>% reduce(left_join, by = "id")

# format and extract stress durations to include as covariate in fMRI analysis
cov <- stress %>%
  group_by(id, run, condition)  %>% 
  summarise(total_stress_dur = sum(Stress_duration)) %>% 
  mutate(condition =  factor(condition, levels = c("con", "noc", "bas"))) %>% # order rows to match GLM setup for fMRI analysis
  arrange(id, run, condition) 
write.table(cov[, 4], "../anonymised_data/stressDur.csv", sep="\t", col.names=F, row.names=F)
```
...
aversLow <- stress_ratings %>% filter(! aversiveness<25) %>% 
...
stress_ratings$stress_dur_diff.z <- scale_this(stress_ratings$stress_dur_diff) # mean center stress dur difference

# maximal model
avers.max <- mixed(aversiveness ~ condition*run.z + (1+condition*run.z+stress_dur_diff.z|id), data=stress_ratings, method = "S",
                   control = lmerControl(optimizer="bobyqa", optCtrl = list(maxfun=10000)))
summary(avers.max) # singular fit

# remove correlation of random slopes and intercepts
avers.m1 <- mixed(aversiveness ~ condition*run.z + (1+condition*run.z+stress_dur_diff.z||id), data=stress_ratings, method = "S",
...
avers.m2 <- mixed(aversiveness ~ condition*run.z + (1+condition+run.z+stress_dur_diff.z||id), data=stress_ratings, method = "S",
                  control = lmerControl(optimizer="bobyqa", optCtrl = list(maxfun=10000)), expand_re=T)
...
avers.m3 <- mixed(aversiveness ~ condition*run.z + (1+run.z+stress_dur_diff.z||id), data=stress_ratings, method = "S",
                  control = lmerControl(optimizer="bobyqa", optCtrl = list(maxfun=10000)))
summary(avers.m3) # singular fit

# remove random slopes for stress duration difference
avers.m4 <- mixed(aversiveness ~ condition*run.z + (1+run.z|id), data=stress_ratings, method = "S",
                  control = lmerControl(optimizer="bobyqa", optCtrl = list(maxfun=10000)))
summary(avers.m4) # model converged, no warnings

# diagnostic plot for reduced model (use lmer because "fitted" doesn't work on afex model)
avers.m4d <- lmer(aversiveness ~ condition*run.z + (1+run.z|id), data=stress_ratings, method = "S",
                  control = lmerControl(optimizer="bobyqa", optCtrl = list(maxfun=10000)))
plot(avers.m4d)

# plot random effects
lattice::dotplot(ranef(avers.m4d))

# compare maximal and reduced model
avers.m4c <- update(avers.m4, REML=F); avers.maxc <- update(avers.max, REML=F)
anova(avers.m4c, avers.maxc) # AIC and BIC indicate better fit for reduced model, but no sig. difference

# compare results for maximal and reduced model
avers.results <- left_join(nice(avers.max), nice(avers.m4), by = "Effect", suffix = c("_full", "_final")) # results are very similar
...
# maximal model
ctrl.max <- mixed(control ~ condition*run.z + (1+condition*run.z+stress_dur_diff.z|id), data=stress_ratings, method = "S", 
             control = lmerControl(optimizer="bobyqa", optCtrl = list(maxfun=10000)))
summary(ctrl.max) # singular fit

# remove correlation of random slopes and intercepts
ctrl.m1 <- mixed(control ~ condition*run.z + (1+condition*run.z+stress_dur_diff.z||id), data=stress_ratings, method = "S", 
             control = lmerControl(optimizer="bobyqa", optCtrl = list(maxfun=10000)))
summary(ctrl.m1) # model failed to converge

# print random effects terms to inform further model reduction
summary(ctrl.m1)$varcor 

# remove interaction of random slopes
ctrl.m2 <- mixed(control ~ condition*run.z + (1+condition+run.z+stress_dur_diff.z||id), data=stress_ratings, method = "S", 
             control = lmerControl(optimizer="bobyqa", optCtrl = list(maxfun=10000)))
summary(ctrl.m2) # singular fit

# print random effects terms to inform further model reduction
summary(ctrl.m2)$varcor 

# remove random slopes for stress duration difference and add correlation again
ctrl.m3 <- mixed(control ~ condition*run.z + (1+condition+run.z|id), data=stress_ratings, method = "S", 
             control = lmerControl(optimizer="bobyqa", optCtrl = list(maxfun=10000)))
summary(ctrl.m3) # model converged, no warnings

# diagnostic plot for reduced model (use lmer because "fitted" doesn't work on afex model)
ctrl.m3d <- lmer(control ~ condition*run.z + (1+condition+run.z|id), data=stress_ratings, method = "S", 
             control = lmerControl(optimizer="bobyqa", optCtrl = list(maxfun=10000)))
plot(ctrl.m3d)

# plot random effects
lattice::dotplot(ranef(ctrl.m3d))

# compare maximal and reduced model
ctrl.m3c <- update(ctrl.m3, REML=F); ctrl.maxc <- update(ctrl.max, REML=F)
anova(ctrl.m3c, ctrl.maxc) # BIC suggests reduced model is better, whereas based on AIC maximum model should be favoured (but that one gave warnings)

# compare results for maximal and reduced model
ctrl.results <- left_join(nice(ctrl.max), nice(ctrl.m3), by = "Effect", suffix = c("_full", "_final")) # results are very similar
...
ratings$stress_dur_diff.z <- scale_this(ratings$stress_dur_diff) # mean center stress duration difference

# maximal model
stress.max <- mixed(stress ~ condition + (1+condition*run.z + stress_dur_diff.z|id), data=ratings, method = "S", 
             control = lmerControl(optimizer="bobyqa", optCtrl = list(maxfun=10000)))
summary(stress.max) # singular fit

# remove correlation of random slopes and intercept
stress.m1 <- mixed(stress ~ condition + (1+condition*run.z+stress_dur_diff.z||id), data=ratings, method = "S", 
             control = lmerControl(optimizer="bobyqa", optCtrl = list(maxfun=10000)), expand_re=T)
...
# remove interaction of random slopes
stress.m2 <- mixed(stress ~ condition + (1+condition+run.z+stress_dur_diff.z||id), data=ratings, method = "S", 
             control = lmerControl(optimizer="bobyqa", optCtrl = list(maxfun=10000)))
summary(stress.m2) # singular fit

# print random effects terms to inform further model reduction
summary(stress.m2)$varcor 

# remove random slopes for stress duration difference and add correlation again
stress.m3 <- mixed(stress ~ condition + (1+condition+run.z|id), data=ratings, method = "S", 
             control = lmerControl(optimizer="bobyqa", optCtrl = list(maxfun=10000)))
summary(stress.m3) # model converged, no warnings

# diagnostic plot for reduced model (use lmer because "fitted" doesn't work on afex model)
stress.m3d <- lmer(stress ~ condition + (1+condition+run.z|id), data=ratings, method = "S", 
             control = lmerControl(optimizer="bobyqa", optCtrl = list(maxfun=10000)))
plot(stress.m3d)

# plot random effects
lattice::dotplot(ranef(stress.m3d))

# compare maximal and reduced model
stress.m3c <- update(stress.m3, REML=F); stress.maxc <- update(stress.max, REML=F)
anova(stress.m3c, stress.maxc) # reduced model fits significantly better

# compare results for maximal and reduced model
stress.results <- left_join(nice(stress.max), nice(stress.m3), by = "Effect", suffix = c("_full", "_final")) # condition effect holds up no matter the model

# post-hoc contrasts
emm_stress <- emmeans(stress.m3, "condition")
emm_stress
update(pairs(emm_stress), by = NULL, adjust = "holm")
# sig. higher stress ratings in stress compared to baseline, only trend difference between con and noc
...
stress.m3avers <- mixed(stress ~ condition*aversiveness.z + (1+condition*aversiveness.z+run.z|id) + (1|run), data=stress_ratings, method = "S", 
             control = lmerControl(optimizer="bobyqa", optCtrl = list(maxfun=10000)))
summary(stress.m3avers) # no interaction between condition and aversiveness
```
...
# maximal model
help.max <- mixed(helplessness ~ condition + (1+condition*run.z+stress_dur_diff.z|id), data=stress_ratings, method = "S", 
             control = lmerControl(optimizer="bobyqa", optCtrl = list(maxfun=10000)))
summary(help.max) # model converged, no warnings

# diagnostic plot for reduced model (use lmer because "fitted" doesn't work on afex model)
help.maxd <- lmer(helplessness ~ condition + (1+condition*run.z+stress_dur_diff.z|id), data=stress_ratings, method = "S", 
             control = lmerControl(optimizer="bobyqa", optCtrl = list(maxfun=10000))) 
...
stressr <- list(stressr, stressDur[, c(1,4)]) %>% reduce(left_join, by = "id") # add stress duration difference
stressr$stress_dur_diff.z <- scale_this(stressr$stress_dur_diff) # mean center stress duration difference
stressr$run.z <- scale_this(stressr$run) # mean center run

# maximal model
rt.max <- mixed(rt ~ condition + (1+condition*run.z+stress_dur_diff.z|id), data=stressr, method = "S", 
             control = lmerControl(optimizer="bobyqa", optCtrl = list(maxfun=10000)))
summary(rt.max) # singular fit

# remove correlation of random slopes and intercepts by id
rt.m1 <- mixed(rt ~ condition + (1+condition*run.z+stress_dur_diff.z||id), data=stressr, method = "S",
             control = lmerControl(optimizer="bobyqa", optCtrl = list(maxfun=10000)), expand_re=T)
summary(rt.m1) # singular fit

# remove interaction of random slopes
rt.m2 <- mixed(rt ~ condition + (1+condition+run.z+stress_dur_diff.z||id), data=stressr, method = "S",
             control = lmerControl(optimizer="bobyqa", optCtrl = list(maxfun=10000)), expand_re=T)
...
# remove random slopes for condition and add correlations again
rt.m3 <- mixed(rt ~ condition + (1+run.z+stress_dur_diff.z|id), data=stressr, method = "S",
             control = lmerControl(optimizer="bobyqa", optCtrl = list(maxfun=10000)))
summary(rt.m3) # model converged, no warnings

# diagnostic plot for reduced model (use lmer because "fitted" doesn't work on afex model)
rt.m3d <- lmer(rt ~ condition  + (1+run.z+stress_dur_diff.z|id), data=stressr, method = "S",
             control = lmerControl(optimizer="bobyqa", optCtrl = list(maxfun=10000)))
plot(rt.m3d)
...
anova(rt.m3c, rt.maxc) # reduced model shows significantly better fit
...
stressc <- list(stressc, stressDur[, c(1,4)]) %>% reduce(left_join, by = "id") # add stress duration difference
stressc$stress_dur_diff.z <- scale_this(stressc$stress_dur_diff) # mean center stress duration difference
stressc$run.z <- scale_this(stressc$run) # mean center run

# maximal model
corr.max <- mixed(correct ~ condition + (1+condition*run.z+stress_dur_diff.z|id), data=stressc, family = binomial, method="LRT",
                  control = glmerControl(optimizer="bobyqa", optCtrl = list(maxfun=10000)))
summary(corr.max) # singular fit              

# remove correlation of random slopes and intercepts by id
corr.m1 <- mixed(correct ~ condition + (1+condition*run.z+stress_dur_diff.z||id), data=stressc, family = binomial, method="LRT",
...
# remove interaction of random slopes
corr.m2 <- mixed(correct ~ condition + (1+condition+run.z+stress_dur_diff.z||id), data=stressc, family = binomial, method="LRT",
                  control = glmerControl(optimizer="bobyqa", optCtrl = list(maxfun=10000)))
summary(corr.m2) # singular fit

# print random effects terms to inform further model reduction
summary(corr.m2)$varcor

# remove random slopes for run
corr.m3 <- mixed(correct ~ condition + (1+condition+stress_dur_diff.z||id), data=stressc, family = binomial, method="LRT",
                  control = glmerControl(optimizer="bobyqa", optCtrl = list(maxfun=10000)))
summary(corr.m3) # singular fit

# remove random slopes for condition
corr.m4 <- mixed(correct ~ condition + (1+stress_dur_diff.z||id), data=stressc, family = binomial, method="LRT",
                  control = glmerControl(optimizer="bobyqa", optCtrl = list(maxfun=10000)))
summary(corr.m4) # model converged, no warnings

# diagnostic plot for reduced model (use lmer because "fitted" doesn't work on afex model)
corr.m4d <- glmer(correct ~ condition + (1+stress_dur_diff.z||id), data=stressc, family = binomial,
                  control = glmerControl(optimizer="bobyqa", optCtrl = list(maxfun=10000)))
plot(corr.m4d) # ceiling effect?!

# plot random effects
lattice::dotplot(ranef(corr.m4d))

# compare maximal and reduced model
corr.m4c <- update(corr.m4, REML=F); corr.maxc <- update(corr.max, REML=F)
anova(corr.m4c, corr.maxc) # reduced model fits significantly better

# compare results for maximal and reduced model
corr.results <- left_join(nice(corr.max), nice(corr.m4), by = "Effect", suffix = c("_full", "_final")) 

# post-hoc contrasts
emm_corr <- emmeans(corr.m4, "condition")
emm_corr
...
hr <- list(hr, stressDur[, c(1,4)]) %>% reduce(left_join, by = c("id")); hr$id <- as.factor(hr$id) # add stress duration difference
hr$stress_dur_diff.z <- scale_this(hr$stress_dur_diff) # mean center stress duration difference
hr$run.z <- scale_this(hr$run) # mean center run

# maximal model
hr.max <- mixed(bpm ~ condition + (1+condition*run.z+stress_dur_diff.z|id), data=hr, method = "S", 
             control = lmerControl(optimizer="bobyqa", optCtrl = list(maxfun=10000)))
summary(hr.max) # singular fit

# remove correlation of random slopes and intercepts by id
hr.m1 <- mixed(bpm ~ condition + (1+condition*run.z+stress_dur_diff.z||id), data=hr, method = "S", 
             control = lmerControl(optimizer="bobyqa", optCtrl = list(maxfun=10000)), expand_re=T)
summary(hr.m1) # singular fit

# print random effects terms to inform further model reduction
summary(hr.m1)$varcor

# remove random slopes for condition
hr.m2 <- mixed(bpm ~ condition + (1+run.z+stress_dur_diff.z||id), data=hr, method = "S", 
             control = lmerControl(optimizer="bobyqa", optCtrl = list(maxfun=10000)), expand_re=T)
summary(hr.m2) # model converged, no warnings

# diagnostic plot for reduced model (use lmer because "fitted" doesn't work on afex model)
hr.m2d <- lmer(bpm ~ condition + (1+run.z+stress_dur_diff.z||id), data=hr, method = "S", 
             control = lmerControl(optimizer="bobyqa", optCtrl = list(maxfun=10000)))
plot(hr.m2d)

# plot random effects
lattice::dotplot(ranef(hr.m2d))

# compare maximal and reduced model
hr.m2c <- update(hr.m2, REML=F); hr.maxc <- update(hr.max, REML=F)
anova(hr.m2c, hr.maxc) # reduced model fits better, but no sig. difference

# compare results for maximal and reduced model
hr.results <- left_join(nice(hr.max), nice(hr.m2), by = "Effect", suffix = c("_full", "_final")) # condition effect only in reduced model

# post-hoc contrasts
emm_hr <- emmeans(hr.m2, "condition")
emm_hr
...
# MR Parameter Estimates from vmPFC

```{r plot MR parameter estimates from vmPFC}
# compute summary statistics for conditions and runs
...
betasBar <- ggplot(betasCondRun_sumstats, aes(x=run, y=beta_weight, fill=condition)) +
  ...
  laura_theme() +
  scale_fill_manual(values=c(colour1, colour2, colour3), labels=c("BASE", "CON", "UNCON"))
betasBar
```
...
# mean center beta weights
ratings$beta_weight.z <- scale_this(ratings$beta_weight) 
stress_ratings$beta_weight.z <- scale_this(stress_ratings$beta_weight) 

# stress
stress.m3Beta <- mixed(stress ~ condition*beta_weight.z + (1+condition+run.z|id), data=ratings, method = "S", 
                       control = lmerControl(optimizer="bobyqa", optCtrl = list(maxfun=10000)))
summary(stress.m3Beta) # no modulation

stressBeta.results <- nice(stress.m3Beta)

# post-hoc contrasts of slopes
emtrends(stress.m3Beta, pairwise ~ condition, var = "beta_weight.z")
emmip(stress.m3Beta, condition ~ beta_weight.z, cov.reduce = range)

# helplessness
help.maxBeta <- mixed(helplessness ~ condition*beta_weight.z + (1+condition*run.z+stress_dur_diff.z|id), data=stress_ratings, method = "S", 
             control = lmerControl(optimizer="bobyqa", optCtrl = list(maxfun=10000)))
summary(help.maxBeta) # singular fit

# print random effects terms to inform further model reduction
summary(help.maxBeta)$varcor 

# remove interaction of random slopes
help.maxBeta2 <- mixed(helplessness ~ condition*beta_weight.z + (1+condition+run.z+stress_dur_diff.z|id), data=stress_ratings, method = "S", 
             control = lmerControl(optimizer="bobyqa", optCtrl = list(maxfun=10000)))
summary(help.maxBeta2) # singular fit

# remove random slopes for run
help.maxBeta3 <- mixed(helplessness ~ condition*beta_weight.z + (1+condition+stress_dur_diff.z|id), data=stress_ratings, method = "S", 
             control = lmerControl(optimizer="bobyqa", optCtrl = list(maxfun=10000)))
summary(help.maxBeta3) # singular fit

# remove random slopes for condition
help.maxBeta4 <- mixed(helplessness ~ condition*beta_weight.z + (1+stress_dur_diff.z|id), data=stress_ratings, method = "S", 
             control = lmerControl(optimizer="bobyqa", optCtrl = list(maxfun=10000)))
summary(help.maxBeta4) # model converged, no warnings

# compare results for maximal and reduced model
helpBeta.results <- left_join(nice(help.maxBeta), nice(help.maxBeta4), by = "Effect", suffix = c("_full", "_final")) 

# post-hoc contrast of slopes
emtrends(help.maxBeta4, pairwise ~ condition, var = "beta_weight.z")
emmip(help.maxBeta4, condition ~ beta_weight.z, cov.reduce = range)

# save table with model details (includes confidence intervals, marginal and conditional R2)
tab_model(help.maxBeta4,
          string.ci = "95% CI",
          dv.labels = c("Fixed Effects - Helplessness"),
          pred.labels = c("Intercept", 
                          "Condition",
...
p.orig <- c(.0845, .0001, .0001, .0073, .1159) # p-values in order of DVs
